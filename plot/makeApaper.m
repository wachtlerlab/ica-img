function [ dirpath ] = makeApaper(M_plain, Act_csr)
...
fh = plotActSingleFit(Act_csr, bf, imgnr);
...
fh = plotActSingleImg(Act_csr, bf, imgnr);
figname = sprintf('%s-img-%d-%d.eps', basepath, bf, imgnr);
set(fh, 'PaperUnits', 'centimeters', 'PaperSize', [4.25 4.25], 'PaperPosition', [0, 0, 4.25, 4.25])
print(fh, '-depsc2', '-r300', '-loose', figname)

fh = plotActSingleATA(Act_csr, bf, imgnr);
figname = sprintf('%s-ata-%d-%d.eps', basepath, bf, imgnr);
set(fh, 'PaperUnits', 'centimeters', 'PaperSize', [4.25 4.25], 'PaperPosition', [0, 0, 4.25, 4.25])
print(fh, '-depsc2', '-r300', '-loose', figname)


fh = plotAImgData(Act_csr.Model, imgnr);
...
fh = plotACoeffStats(A, Act_csr);
print(fh, '-depsc2', '-r300', '-loose', [basepath '-coeff.eps'])

A = AfromModel(M_plain);
basepath = fullfile(dirpath, A.name);
...
fh = plotAspace(A);
%set(fh, 'PaperPositionMode', 'auto');
print(fh, '-depsc2', '-r300', '-loose', [basepath '-space.eps'])
